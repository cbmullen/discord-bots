...
export async function handlePlayerSelectSimultaneous(env, interaction, customObj) {
  const selectedUsersList = GetSelectedUsers(interaction)
...
export async function handlePlayerSorting(env, interaction, customObj) { 
  ...
  let response = SendStringSelectMessage(`Select Player ${orderedUserList.length + 1}`, interaction.data.custom_id, options);
